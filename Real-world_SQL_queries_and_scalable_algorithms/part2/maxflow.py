...
def maxflow(bfs_max_iterations=float('inf'), flow_max_iterations=float('inf')):
    ...
    while True:
        ...
        while not sink_path_found:
            ...
            db.execute("""
                    SELECT array_append(p.path, e.id) AS path, array_append(p.nodes, e.dst) AS nodes
                    INTO tmp
                    FROM edge e, paths p
                    WHERE e.src = p.nodes[array_length(p.nodes, 1)]
                          AND e.capacity != 0
        ...
        db.execute("""
            ...
            constraining_capacity(capacity) AS (
                ...
                FROM edge e, path_edges 
                WHERE path_edges.path_edge = e.id
            )
        ...
        db.execute("""
            WITH updates(id, new_capacity) AS (
                ...
                FROM flow_to_route f, flip_edge fl, edge e
                WHERE f.edge_id = e.id
                UNION
                SELECT fl.reverse_id, e.capacity + f.flow
                FROM flow_to_route f, flip_edge fl, edge e
                WHERE f.edge_id = e.id AND fl.forward_id = e.id
            )
            UPDATE edge
