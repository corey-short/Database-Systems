...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    // IMPLEMENT ME
    if (inputClosed) {
      throw new SparkException("Should not be inserting row into closed input. Bad things will happen!")
    }
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      val inFile = Files.newInputStream(path)
      ...
      private[this] def fetchNextChunk(): Boolean = {
        ...
        if (chunkSizeIterator.hasNext) {
          byteArray = getNextChunkBytes(inFile, chunkSizeIterator.next(), byteArray)
  ...
  def closeInput() = {
    ...
    //TODO: close outStream no matter what or inside our if above
    outStream.close()
  }
  /**
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    for (i <- 0 until size) {
      val DiskPartition = new DiskPartition(Integer.toString(i), blockSize)
      partitions(i) = DiskPartition
    }

    for (row <- input) {
      val hash = keyGenerator.apply(row).hashCode() % size
      partitions(hash).insert(row)
    }
    for (i <- 0 until size) {
