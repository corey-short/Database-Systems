...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENT ME
    val diskPartitions = DiskHashedRelation(input, keyGenerator)
    val partitionIterator = diskPartitions.getIterator()
    val cacheGenerator = CS186Utils.generateCachingIterator(projectList, child.output)
    ...
    new Iterator[Row] {
      def hasNext() = {
        ...
        if (cacheIterator == null || !cacheIterator.hasNext) {
          val done: Boolean = fetchNextPartition()
          if (!done) {
            diskPartitions.closeAllPartitions()
          }
          done
        } else { true }
      }
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        if (partitionIterator.hasNext) {
          cacheIterator = cacheGenerator(partitionIterator.next().getData())
