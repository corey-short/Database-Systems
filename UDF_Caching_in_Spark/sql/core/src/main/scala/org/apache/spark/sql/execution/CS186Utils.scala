...
object CS186Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    // IMPLEMENT ME
    var finaludf: ScalaUdf = null
    if (expressions.size > 0) {
      for (exp <- expressions) {
        exp match {
          case isUdf: ScalaUdf => finaludf = isUdf
          case _ =>
        }
      }
    }
    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          // IMPLEMENT ME
          val row = input.next()
          val pre = preUdfProjection.apply(row)
          val post = postUdfProjection.apply(row)
          val key = cacheKeyProjection.apply(row)
          if (cache.containsKey(key)) {
            Row.fromSeq(pre ++ cache.get(key) ++ post)
          }
          else {
            val evaluatedUdf = udfProject.apply(row)
            cache.put(key, evaluatedUdf)
            Row.fromSeq(pre ++ evaluatedUdf ++ post)
          }
        }
      }
    }
