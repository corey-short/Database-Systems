 ...
 *
 * @params {Array} data is a list of objects containing the keys 'state' and 'total_amount'
 */
USCashMap.prototype.render = function (data) {
    ...
    this.states.on('mouseover', function(d, e, p){
        ...
        //console.log("Component received a mouseover event!");  // Remove when implemented
                                                                                        // (1)
        if (!that.hasStateInSelection(d['state'])) { that.addStateToSelection(d['state']); }
    ...
    this.states.on('mouseout', function(d, e, p){
        ...
        that.clearInspectionInfo();                                                     // (1)
                                                                                        // (2)
        if (that.hasStateInSelection(d['state'])) { that.removeStateFromSelection(d['state']); }

        var mouseOut = this;
        d3.select(mouseOut)
            .style("fill", function(d) {
                return moneyColorScale(d['total_amount']);                              // (3)
            });
    ...
    this.states.on('click', function(d, e, p){
        ...
        d3.select(this).style("fill", "#B44E86");
        that.setInspectionInfo(that.stateNameMap.get(['state']), d['total_amount']);
