...
TransactionHistogram.prototype.render = function(data) {
    ...
    // Implement
    // Add a new grouping
    var enterGroup = bar.enter().append("g")
        .attr("class", "bar")
    ...
    enterGroup.append("rect")
        ...
        .attr("y", 0)
        .style("fill", function() {
            if (that.currentColorState == that.colorStates.PRIMARY) { return "#B44E86"; }
            else if (that.currentColorState == that.colorStates.SECONDARY) { return "#15781B"; }
            else { return "#FFCC66"; }
        })
    ...
    enterGroup.append("text")
        .attr("x", (histogramData[0].dx / 2) - 7)
        .attr("y", function(d) {
            if (that.isBarLargeEnough(d)) { return 1; } else { return -15 }
        })
        .attr("dy", "1.5em")
    ...
    /** Update phase */
    // Implement
    var updateGroup = bar.transition()
        .duration(750)
        .attr("transform", function(d) {
            return "translate(" + that.xScale(d.x) + "," + that.yScale(d.y) + ")";
        });
