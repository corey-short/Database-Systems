...
def get_all_transaction_amounts():
    ...
    # Execute database query
    db.execute("SELECT cc.state, cc.transaction_amt FROM committee_contributions AS cc WHERE cc.transaction_amt > 0::numeric;")
    results = db.fetchall()

    # Package into output
    return [{'state': row[0], 'amount': row[1]} for row in results]
...
def get_total_transaction_amounts_by_state():
    ...
    """
    # Execute database query
    db.execute("SELECT cc.state, SUM(cc.transaction_amt) FROM committee_contributions AS cc WHERE cc.transaction_amt > 0::numeric GROUP BY cc.state;")
    results = db.fetchall()
    # Package into list of dictionaries
    return [{'state': row[0], 'total_amount': row[1]} for row in results]
